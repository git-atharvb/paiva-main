  ...
  return (
    <Card elevated className="flex flex-col h-full rounded-2xl md:rounded-[2rem] p-6 lg:p-8">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10">
        ...
        <div className="bg-linear-to-br from-primary to-blue-600 text-white self-end p-5 px-7 rounded-2xl rounded-br-sm shadow-md max-w-[85%] leading-relaxed animate-in fade-in slide-in-from-bottom-2 duration-500" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
          Please help me draft a massive UI upgrade plan.
        </div>
        <div className="bg-white/60 dark:bg-black/60 text-foreground self-start p-5 px-7 rounded-2xl rounded-bl-sm border border-white/20 shadow-sm max-w-[85%] leading-relaxed animate-in fade-in slide-in-from-bottom-2 duration-500" style={{ animationDelay: '400ms', animationFillMode: 'both' }}>
      ...
      <form className="flex flex-col sm:flex-row gap-4 mt-6 lg:mt-8" onSubmit={e => e.preventDefault()}>
        ...
        />
        <Button type="submit" variant="primary" size="icon" className="rounded-full size-12 shrink-0 shadow-premium hover:shadow-neon transition-shadow">Send</Button>
      </form>
    </Card>
  );
}